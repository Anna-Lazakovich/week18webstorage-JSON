...
function makeTwo() {
	localStorage.setItem('score', '7,6,5');
...
function makeThree() {
	localStorage.setItem('characteristic', 'name=John, age=25');
...
const showEight = () => {
	const arrayEightString = localStorage.getItem('objectEight');
	if (arrayEightString) {
		const arrayEight = JSON.parse(arrayEightString);
		const resultElement = document.querySelector('.practicum15');
		for (const key in arrayEight) {
			const value = arrayEight[key];
...
const showSeven = () => {
	const arraySevenString = localStorage.getItem('objectSeven');
	const arraySeven = JSON.parse(arraySevenString);
	if (arraySevenString) {
		const resultElement = document.querySelector('.practicum16');
		resultElement.innerHTML += `В ${arraySeven['city']} живет ${arraySeven['population']} человек; `;
	} else {
		console.log('Массив arraySeven не найден в Local Storage.');
	}
};
...
//Задание 19
//Cоздана форма с одним поле ввода, в которое пользователь может вводить только числа. Создайте массив numbers. Когда пользователь нажимает кнопке Задание 19 число должно добавляться в массив. Массив должен сохраняться в Local Storage с ключом task19. Вызывается функция по кнопке Задание 19. Что произойдёт, если нажать на кнопке трижды?
...
//Если нажать на кнопку трижды, то в массив добавится NaN трижды.

//Задание 20
//Создайте функцию removeLastElement, которая при вызове удаляет из массива numbers последний элемент. После чего массив сохраняется в Local Storage с ключем task20 (используйте массив из задания 19). Вызывается функция по кнопке Задание 20.

const removeLastElement = () => {
	if (numbers.length > 0) {
		const lastElement = numbers.pop();
		localStorage.setItem('task20', lastElement);
...
const showCart = () => {
	...
	const task21 = JSON.parse(localStorage.getItem('task21'));
	practicum23.innerHTML += task21 + ' ';
};
...
const updateCartCount = () => {
	...
	practicum24.innerHTML = `Количество товаров в корзине ${count}`;
};
...
const setCookie = (name, value) => {
	...
	console.log(`Cookie ${name}=${value} установлен.`);
};
